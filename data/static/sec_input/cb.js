...
Handsontable.hooks.add('afterChange', function(changes, source){
  if (source != 'loadData'){
    ...
    if (changes[0][1] == 3 && changes[0][3] == '기타'){
        ...
        if (pgcode_source != null && pgcode_source != ''){
...
function loadinfo(bond_id, edittime, editor){
  ...
  if (check_copyandpaste == true){
	...
	var h = document.getElementById('secheader');    // secheader : c:/kisgiga/pricer/templates/sec_input/유형.html - <h4 id = "secheader"></h4>
	$.ajax({
    ...
    if (edittime != 'none' && edittime != undefined) {
      ...
      //calllimit_highlight
      // calllimit_hot.setCellMeta(0,0,'changed',data.highlights.calllimit_highlight[0])
