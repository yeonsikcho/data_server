...
var bond_hot = new Handsontable(container, {licenseKey:'non-commercial-and-evaluation',
  ...
  rowHeaderWidth: 150,
  rowHeaders: ['BondID','발행일','만기일','배당주기','배당율','만기수익률','First_Coupon','Last_Coupon','발행주당가','누적적 우선주','참가적 우선주','리픽싱 조항'], colHeaders: false,
  contextMenu: true,
  colWidths: [250],
  className: "htCenter",
  ...
  hooks_change = true;
    ...
    if (data.msg != null){
      ...
      return
		...
		if (convstock_hot.getDataAtCell(0,0) == null){convstock_hot.setDataAtCell(0,0,bond_hot.getDataAtCell(0,0).split("|")[0])}
    ...
    if (edittime != 'none' && edittime != undefined) {
      ...
      conversion_hot.updateSettings({cells:function(row, col, prop){
        ...
        for (r=0; r<conversion_hot.countRows(); r++){
          for (c=0; c<conversion_hot.countCols(); c++){//conversion_hot.setCellMeta(r, c, 'changed', data.highlights.conversion_highlights[r][c]);}
            if (data.highlights.conversion_highlights[r][c] != null){
              if(row==r & col==c){cellProperties.className = 'set_yellow';} 
            }
          }
        }
        return cellProperties
      ...
      //calllimit_highlight
      calllimit_hot.updateSettings({cells:function(row, col, prop){// calllimit_hot.setCellMeta(0,0,'changed',data.highlights.calllimit_highlight[0])
       ...
       putcall_hot.updateSettings({cells:function(row, col, prop){
        ...
        for (r=0; r<putcall_hot.countRows(); r++){
          for (c=0; c<putcall_hot.countCols(); c++){//putcall_hot.setCellMeta(r, c, 'changed', data.highlights.putcall_highlights[r][c]);}
            if (data.highlights.putcall_highlights[r][c] != null){
              if(row==r & col==c){cellProperties.className = 'set_yellow';}
            }
          }
        }
      ...
      putcalldes_hot.updateSettings({cells:function(row, col, prop){
        ...
        for (r=0; r<putcalldes_hot.countRows(); r++){
          for (c=0; c<putcalldes_hot.countCols(); c++){//putcalldes_hot.setCellMeta(r, c, 'changed', data.highlights.putcalldes_highlights[r][c]);}
            if (data.highlights.putcalldes_highlights[r][c] != null){
              if(row==r & col==c){cellProperties.className = 'set_yellow';}
            }
          }
        }
      ...
      bondschedule_hot.updateSettings({cells:function(row, col, prop){
        ...
        for (r=0; r<bondschedule_hot.countRows(); r++){
          for (c=0; c<bondschedule_hot.countCols(); c++){//bondschedule_hot.setCellMeta(r, c, 'changed', data.highlights.bondschedule_highlights[r][c]);}
            if (data.highlights.bondschedule_highlights[r][c] != null){
              if(row==r & col==c){cellProperties.className = 'set_yellow';}
            }
          }
        }
      ...
      optionschedule_hot.updateSettings({cells:function(row, col, prop){
        ...
        for (r=0; r<optionschedule_hot.countRows(); r++){
          for (c=0; c<optionschedule_hot.countCols(); c++){//optionschedule_hot.setCellMeta(r, c, 'changed', data.highlights.optionschedule_highlights[r][c]);}
            if (data.highlights.optionschedule_highlights[r][c] != null){
              if(row==r & col==c){cellProperties.className = 'set_yellow';}
            }
          }
        }
      ...
      bond_hot.updateSettings({cells:function(row, col, prop){
        var cellProperties = {};
        if (row<2){cellProperties.className = 'set_render', cellProperties.readOnly = 'true'};
        if (row>=9){cellProperties.type = 'dropdown', cellProperties.source = ['Y','N']};
        return cellProperties
      }})
    }
